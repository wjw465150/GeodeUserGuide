...
<a name="34______为分区区域配置成员崩溃冗余恢复"></a>
##### 为分区区域配置成员崩溃冗余恢复
...
<a name="65____如何解决Destroy和Clear操作"></a>
### 如何解决Destroy和Clear操作
...
<a name="68____Persistence_and_Overflow（持久性和溢出）"></a>
### Persistence and Overflow（持久性和溢出）
...
<a name="86_____Geode_PDX序列化功能"></a>
#### Geode PDX序列化功能
...
<a name="88_____使用基于自动反射的PDX序列化"></a>
#### 使用基于自动反射的PDX序列化
...
<a name="89______使用类模式字符串自定义序列化"></a>
##### 使用类模式字符串自定义序列化
...
<a name="90______扩展ReflectionBasedAutoSerializer"></a>
...
<a name="91_____使用PdxSerializer序列化您的域对象"></a>
#### 使用PdxSerializer序列化您的域对象
...
<a name="92_____在域对象中实现PdxSerializable"></a>
#### 在域对象中实现PdxSerializable
...
<a name="93_____编写应用程序以使用PdxInstances"></a>
#### 编写应用程序以使用PdxInstances
...
<a name="94_____将JSON文档添加到Geode缓存"></a>
#### 将JSON文档添加到Geode缓存
...
<a name="95_____使用PdxInstanceFactory创建PdxInstances"></a>
#### 使用PdxInstanceFactory创建PdxInstances
...
<a name="97_____使用PDX对象作为区域输入键"></a>
#### 使用PDX对象作为区域输入键
...
<a name="98____Geode数据序列化（DataSerializable和DataSerializer）"></a>
...
<a name="108_____为Write_Behind_Cache事件处理实现AsyncEventListener"></a>
#### 为Write-Behind Cache事件处理实现AsyncEventListener
...
<a name="109_____如何从事件处理程序回调安全地修改缓存"></a>
#### 如何从事件处理程序回调安全地修改缓存
...
<a name="114______高度可用的客户端服务器事件消息"></a>
##### 高度可用的客户端服务器事件消息
...
<a name="115_____实施持久的客户端服务器消息传递"></a>
#### 实施持久的客户端服务器消息传递
...
<a name="118______限制服务器的订阅队列内存使用"></a>
##### 限制服务器的订阅队列内存使用
...
<a name="119______调整客户端的订阅消息跟踪超时"></a>
##### 调整客户端的订阅消息跟踪超时
...
<a name="120____配置多站点（WAN）事件队列"></a>
### 配置多站点（WAN）事件队列
...
<a name="122_____为事件分发配置Dispatcher线程和顺序策略"></a>
#### 为事件分发配置Dispatcher线程和顺序策略
...
<a name="135_____在Geode中编写和执行查询"></a>
#### 在Geode中编写和执行查询
...
<a name="137______IMPORT_Statement（IMPORT语句）"></a>
##### IMPORT Statement（IMPORT语句）
...
<a name="138______FROM_Clause（FROM子句）"></a>
##### FROM Clause（FROM子句）
...
<a name="139______WHERE_Clause（WHERE子句）"></a>
##### WHERE Clause（WHERE子句）
...
<a name="140______SELECT_Statement（SELECT语句）"></a>
##### SELECT Statement（SELECT语句）
...
<a name="151_____查询语言限制和不受支持的特性"></a>
#### 查询语言限制和不受支持的特性
...
<a name="158______对分区区域使用ORDER_BY"></a>
##### 对分区区域使用ORDER BY
...
<a name="159______在单个节点上查询分区区域"></a>
##### 在单个节点上查询分区区域
...
<a name="160______优化按键或字段值分区的数据查询"></a>
##### 优化按键或字段值分区的数据查询
...
<a name="161______对分区区域执行等连接查询"></a>
##### 对分区区域执行等连接查询
...
<a name="169_____在映射字段上创建索引（映射索引）"></a>
#### 在映射字段上创建索引（映射索引）
...
<a name="171_____维护索引（同步或异步）和索引存储"></a>
#### 维护索引（同步或异步）和索引存储
...
<a name="176_____在使用多个区域的等连接查询上使用索引"></a>
#### 在使用多个区域的等连接查询上使用索引
...
<a name="188____在Apache_Geode中执行一个函数"></a>
### 在Apache Geode中执行一个函数
